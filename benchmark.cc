...
void execute_32_bit(Benchmark benchmark, bool pareto, bool only_mode,
  ...
  if(!benchmark.uses_layout_search()){
    ...
    // check_only("SplayTree", benchmark_32_splay(benchmark, pareto));
  }
  // check_only("StandardBin", benchmark_32_stdbin(benchmark, pareto));
  ...
  // std::cout << "ESEGUO BS A 32BIT" << std::endl;
...
//     check_only("RobinHash", benchmark.template Run<RobinHash<uint64_t>>());
  // std::cout << "ESEGUO BS A 64BIT" << std::endl;
  // check_only("StandardBin", benchmark_64_stdbin(benchmark, pareto));
    ...
    case DataType::UINT32: {
      ...
      if constexpr (sosd_config::fast_mode) {
        ...
        add_search_type("interpolation", execute_32_bit, uint32_t,
                        InterpolationSearch);
        add_search_type("bfs", execute_32_bit, uint32_t,
                        BFS_Search);
    ...
    case DataType::UINT64: {
      ...
      if constexpr (sosd_config::fast_mode) {
        ...
        add_search_type("interpolation", execute_64_bit, uint64_t,
                        InterpolationSearch);
        add_search_type("bfs", execute_64_bit, uint64_t,
                        BFS_Search);
