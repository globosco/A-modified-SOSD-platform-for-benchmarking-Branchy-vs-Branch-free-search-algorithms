...
function do_benchmark() {
    ...
    RESULTS=./results/$1_results_table_EytzingerLayoutNoPrefetch.txt
    if [ -f $RESULTS ]; then
...
function do_benchmark_csv() {
    ...
    RESULTS=./results/$1_results_table_EytzingerLayoutNoPrefetch.csv
    if [ -f $RESULTS ]; then
    ...
    $BENCHMARK -r $2 ./data/$1 ./data/$1_equality_lookups_2M --pareto --csv --search eytzinger_layout_no_prefetch
}
