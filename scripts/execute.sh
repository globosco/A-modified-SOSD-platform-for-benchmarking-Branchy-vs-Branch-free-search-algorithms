...
function do_benchmark() {
    ...
    RESULTS=./results/$1_BinarySearch_results.txt
    if [ -f $RESULTS ]; then
        echo "Already have results for $1"
    else
        echo "Executing workload $1"
        $BENCHMARK -r $2 ./data/$1 ./data/$1_equality_lookups_2M --pareto | tee ./results/$1_results.txt
    fi
}

function do_benchmark_csv() {

    RESULTS=./results/$1_BinarySearch_results_table.csv
    if [ -f $RESULTS ]; then
    ...
    $BENCHMARK -r $2 ./data/$1 ./data/$1_equality_lookups_2M --pareto --csv
}
