...
class Benchmark {
 public:
  Benchmark(const std::string& data_filename,
    ...
    if(uses_layout_search()){
      // std::cout << "Creating Layout" <<  std::endl;
      layout_.resize(data_.size());
      searcher_.createLayout(data_, layout_, 0, data_.size());
  ...
  void Run() {
    ...
    // std::cout << "Executing Index Build..." << std::endl;
 ...
 private:
  ...
  void DoEqualityLookups(Index& index) {
    // std::cout << "Inizio Proccedura dii riceerca" << std::endl;
    if (build_) return;
  ...
  void DoEqualityLookupsCoreLoop(Index& index, unsigned int start,
    ...
    for (unsigned int idx = start; idx < limit; ++idx) {
      ...
      // std::cout << "NUOVO INDICE " << idx << std::endl;
      if constexpr (clear_cache) {
        ...
        number_element += (bound.stop - bound.start);
        reduction_factor += (bound.stop - bound.start)/(double)data_.size()*100;
        queryCounter++;
        uint64_t actual;
        if(!uses_layout_search()){
          const auto bin_start = std::chrono::high_resolution_clock::now();
          actual = searcher_.search(data_, lookup_key, &qualifying,
                                           bound.start, bound.stop);
          const auto bin_end = std::chrono::high_resolution_clock::now();
          bin_time += (std::chrono::duration_cast<std::chrono::nanoseconds>(bin_end - bin_start)
                .count())/(bound.stop - bound.start);
        }else{
          actual = searcher_.search(layout_, lookup_key, &qualifying,
                                           bound.start, bound.stop);
        }
        //CHECK NOT WORK WITH LAYOUTS
        ...
        // std::cout << data_.begin()->key << " " <<  std::prev(data_.end())->key << " " << lookup_key << std::endl;
        // std::cout << "SONO NELL'ELSE" << std::endl;
        if(lookup_key >= std::prev(data_.end())->key){
          iter = std::prev(data_.end());
        }else if(lookup_key <= data_.begin()->key){
          iter = data_.begin();
          
        }else{
          // std::cout << "Executing Lookup..." << std::endl;
          // std::cout << idx << ": " << lookup_key << " -> " << expected << std::endl;
          // not tracking errors, measure the lookup time.
          bound = index.EqualityLookup(lookup_key);
          number_element += (bound.stop - bound.start);
          reduction_factor += (bound.stop - bound.start)/(double)data_.size()*100;
          queryCounter++;
          // std::cout << idx << ": " << bound.start << " " << bound.stop << std::endl;
  ...
  void PrintResult(const Index& index) {
    if (track_errors_) {
      std::cout << "RESULT: " << index.name() << "," << index.variant() << "," 
                << 100-reduction_factor/queryCounter << "," 
                << number_element/queryCounter << ","
                << bin_time/queryCounter << ","
  ...
  void PrintResultCSV(const Index& index) {
    const std::string filename =
        "./results/" + dataset_name_ + "_" + searcher_.name() + "_results_table.csv";
  ...
  std::vector<Row<KeyType>> layout_;
